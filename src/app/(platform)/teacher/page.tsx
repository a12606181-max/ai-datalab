...
import {
  ...
  TeacherLabForm,
  TeacherLessonForm,
} from "@/components/forms/teacher-forms";
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-3 max-w-3xl text-base leading-7 text-white/55">
          Панель показывает результаты студентов, слабые темы группы, последние отправки и позволяет добавлять новые курсы, уроки, лабораторные и датасеты.
        </p>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        <GlassCard>
          <h2 className="text-2xl font-semibold text-white">График результатов студентов</h2>
          <div className="mt-5">
            <LineScoreChart data={analytics.chartData} />
          </div>
        </GlassCard>
        <GlassCard>
          <h2 className="text-2xl font-semibold text-white">Слабые темы группы</h2>
          <div className="mt-5 space-y-3">
            {analytics.weakTopics.map((topic) => (
              <div key={topic.skill} className="rounded-[22px] border border-white/8 bg-white/[0.03] px-4 py-4">
                ...
                <p className="mt-2 text-sm text-white/45">Средний уровень группы: {topic.value}%</p>
              </div>
            ))}
          </div>
        </GlassCard>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <TeacherCourseForm />
        <TeacherLessonForm courses={analytics.courses.map((course) => ({ id: course.id, title: course.title }))} />
        <TeacherLabForm datasets={analytics.datasets.map((dataset) => ({ id: dataset.id, title: dataset.title }))} />
        <TeacherDatasetForm />
      </div>
    </div>
  );
